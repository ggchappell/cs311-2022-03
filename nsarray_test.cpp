...
// Test program for class template NSArray
// For Assignment 2, Exercise A
// Uses the "doctest" unit-testing framework, version 2
// Requires doctest.h, nsarray.h

// Includes for code to be tested
#include "nsarray.h"         // For class template NSArray
#include "nsarray.h"         // Double-inclusion check, for testing only
...
const size_t DEFAULTSIZE = 8;  // Size of default-constructed NSArray<T>
...
TEST_CASE( "Member types (passes if it compiles)" )
{
    ...
    {
        // value_type (NSArray<int>)
        NSArray<int>::value_type x1;
        [[maybe_unused]] int & r1(x1);

        // value_type (NSArray<double>)
        NSArray<double>::value_type x2;
    ...
    {
        // size_type (NSArray<int>)
        NSArray<int>::size_type x1;
        [[maybe_unused]] size_t & r1(x1);

        // size_type (NSArray<double>)
        NSArray<double>::size_type x2;
...
{
    NSArray<int> mi1;
    const NSArray<int> cmi2;
    const NSArray<int> cmi3;
    NSArray<double> md4;
    const NSArray<double> cmd5;

    SUBCASE( "Copy assn returns NSArray by reference" )
    {
        [[maybe_unused]] NSArray<int> & r(mi1 = cmi2);
        REQUIRE( TypeCheck<NSArray<int>>::check(mi1 = cmi2) );
    }

    SUBCASE( "Move assn returns NSArray by reference" )
    {
        NSArray<int> mi_movable1;
        NSArray<int> mi_movable2;
        [[maybe_unused]] NSArray<int> & r(mi1 = move(mi_movable1));
        REQUIRE( TypeCheck<NSArray<int>>::check(mi1 = move(mi_movable2)) );
    }

    SUBCASE( "Bracket op (const) returns item by ref-to-const" )
    {
        // Also check NSArray<T>::value_type
        [[maybe_unused]] const int & r1(cmi2[0]);
        REQUIRE( TypeCheck<int>::check(cmi2[0]) );
        [[maybe_unused]] const NSArray<int>::value_type & r2(cmi2[0]);
        REQUIRE( TypeCheck<NSArray<int>::value_type>::check(cmi2[0]) );

        [[maybe_unused]] const double & r3(cmd5[0]);
        REQUIRE( TypeCheck<double>::check(cmd5[0]) );
        [[maybe_unused]] const NSArray<double>::value_type & r4(cmd5[0]);
        REQUIRE( TypeCheck<NSArray<double>::value_type>::check(cmd5[0]) );
    }
    ...
    SUBCASE( "Bracket op (non-const) returns item by ref" )
    {
        ...
        [[maybe_unused]] NSArray<int>::value_type & r2(mi1[0]);
        REQUIRE( TypeCheck<NSArray<int>::value_type>::check(mi1[0]) );

        [[maybe_unused]] double & r3(md4[0]);
        REQUIRE( TypeCheck<double>::check(md4[0]) );
        [[maybe_unused]] NSArray<double>::value_type & r4(md4[0]);
        REQUIRE( TypeCheck<NSArray<double>::value_type>::check(md4[0]) );
    }
    ...
    SUBCASE( "size returns size_type by value" )
    {
        ...
        [[maybe_unused]] NSArray<int>::size_type && r2(cmi2.size());
        REQUIRE( TypeCheck<NSArray<int>::size_type>::check(cmi2.size()) );

        [[maybe_unused]] size_t && r3(cmd5.size());
        [[maybe_unused]] NSArray<int>::size_type && r4(cmd5.size());
        REQUIRE( TypeCheck<size_t>::check(cmd5.size()) );
        REQUIRE( TypeCheck<NSArray<int>::size_type>::check(cmd5.size()) );
    }
    ...
    SUBCASE( "begin (const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] const NSArray<int>::value_type * && r2(cmi2.begin());
        REQUIRE( TypeCheck<const NSArray<int>::value_type *>::check(cmi2.begin()) );

        [[maybe_unused]] const double * && r3(cmd5.begin());
        REQUIRE( TypeCheck<const double *>::check(cmd5.begin()) );
        [[maybe_unused]] const NSArray<double>::value_type * && r4(cmd5.begin());
        REQUIRE( TypeCheck<const NSArray<double>::value_type *>::check(cmd5.begin()) );
    }
    ...
    SUBCASE( "begin (non-const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] NSArray<int>::value_type * && r2(mi1.begin());
        REQUIRE( TypeCheck<NSArray<int>::value_type *>::check(mi1.begin()) );

        [[maybe_unused]] double * && r3(md4.begin());
        REQUIRE( TypeCheck<double *>::check(md4.begin()) );
        [[maybe_unused]] NSArray<double>::value_type * && r4(md4.begin());
        REQUIRE( TypeCheck<NSArray<double>::value_type *>::check(md4.begin()) );
    }
    ...
    SUBCASE( "end (const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] const NSArray<int>::value_type * && r2(cmi2.end());
        REQUIRE( TypeCheck<const NSArray<int>::value_type *>::check(cmi2.end()) );

        [[maybe_unused]] const double * && r3(cmd5.end());
        REQUIRE( TypeCheck<const double *>::check(cmd5.end()) );
        [[maybe_unused]] const NSArray<double>::value_type * && r4(cmd5.end());
        REQUIRE( TypeCheck<const NSArray<double>::value_type *>::check(cmd5.end()) );
    }
    ...
    SUBCASE( "end (non-const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] NSArray<int>::value_type * && r2(mi1.end());
        REQUIRE( TypeCheck<NSArray<int>::value_type *>::check(mi1.end()) );

        [[maybe_unused]] double * && r3(md4.end());
        REQUIRE( TypeCheck<double *>::check(md4.end()) );
        [[maybe_unused]] NSArray<double>::value_type * && r4(md4.end());
        REQUIRE( TypeCheck<NSArray<double>::value_type *>::check(md4.end()) );
    }
...
{
    SUBCASE( "Taking const NSArray arguments" )
    {

        const NSArray<int> cmi1;
        const NSArray<int> cmi2;

        NSArray<int> mi3(cmi1);         // Copy ctor
        mi3 = cmi1;                     // Copy assn

        [[maybe_unused]] bool result1 = (cmi1 == cmi2);  // op==(NSArray,NSArray)
        [[maybe_unused]] bool result2 = (cmi1 != cmi2);  // op!=(NSArray,NSArray)
        [[maybe_unused]] bool result3 = (cmi1 <  cmi2);  // op<(NSArray,NSArray)
        [[maybe_unused]] bool result4 = (cmi1 <= cmi2);  // op<=(NSArray,NSArray)
        [[maybe_unused]] bool result5 = (cmi1 >  cmi2);  // op>(NSArray,NSArray)
        [[maybe_unused]] bool result6 = (cmi1 >= cmi2);  // op>=(NSArray,NSArray)
...
TEST_CASE( "Comparisons allow min-functionality value types (passes if it compiles)" )
{
    ...
    {
        const NSArray<OnlyEq> cmo1(2);
        const NSArray<OnlyEq> cmo2(2);
    ...
    {
        const NSArray<Counter> cmc1(2);
        const NSArray<Counter> cmc2(2);
...
TEST_CASE( "Checks for noexcept" )
{
    ...
    {
        REQUIRE( noexcept(NSArray<int>(move(mi1))) );
    }
    ...
    {
        REQUIRE( noexcept(mi1.~NSArray()) );
    }

    SUBCASE( "Copy ctor is NOT noexcept" )
    {
        REQUIRE_FALSE( noexcept(NSArray<int>(mi1)) );
    }
    ...
    {
        REQUIRE_FALSE( noexcept(NSArray<int>()) );
    }

    SUBCASE( "Ctor from size is NOT noexcept" )
    {
        REQUIRE_FALSE( noexcept(NSArray<int>(size_t(100))) );
    }
}
...
TEST_CASE( "Size & ctor from size+value" )
{
    ...
    {
        const NSArray<int> cmi(1000000, -63);
...
TEST_CASE( "Bracket operator" )
{
    ...
    using ST = NSArray<int>::size_type;
    const ST thesize = 100;
    NSArray<int> mi1(thesize);
...
TEST_CASE( "Comparisons" )
{
    ...
    {
        // Make vector of 10 NSArray<int> objects, in ascending order
        using VST = vector<NSArray<int>>::size_type;
        auto NUM_ARRAYS = VST(10);
        int A = -2147483641;
        int B = 2147483641;
        vector<NSArray<int>> vv;
...
TEST_CASE( "Same-object tests" )
{
    ...
    {
        NSArray<int> mi_movable(1);
        int * addr1 = &(mi_movable[0]);
        NSArray<int> mi2(move(mi_movable));
        REQUIRE( &(mi2[0]) == addr1 );
    }
    ...
    SUBCASE( "Move assignment returns *this" )
    {
        NSArray<int> mi_movable(1);
        NSArray<int> mi2;
        NSArray<int> & r(mi2 = move(mi_movable));
        REQUIRE( &r == &mi2 );
    }
    ...
    {
        NSArray<int> mi(1);
        NSArray<int>::size_type && rr1(mi.size());
        NSArray<int>::size_type && rr2(mi.size());
        REQUIRE_FALSE( &rr1 == &rr2 );
    }

    SUBCASE( "Bracket op (const) returns original item" )
    {
        const NSArray<int> mi(1);
        const int & r1(mi[0]);
        const int & r2(mi[0]);
        REQUIRE( &r1 == &r2 );
    }
...
TEST_CASE( "Ctor/dctor/assignment count" )
{
    ...
    SUBCASE( "Creation & destruction: ctor/dctor count" )
    {
        ...
        { // Block, so we get dctor calls before function ends
            const NSArray<Counter> mc(thesize);
    ...
    {
        NSArray<Counter> mc1(thesize);
        Counter::reset();

        const NSArray<Counter> mc2(mc1);  // Copy construction
        REQUIRE( Counter::getCtorCount() == thesize );
...
TEST_CASE( "Ctor/dctor/assignment count" )
{
   ...
   Counter::reset();               // Restart all counts
    ...
    {
        {  // Block, so we get dctor calls before function ends
            const NSArray<Counter> mc1(thesize);
    ...
    SUBCASE( "Copy ctor: value-type ctor/dctor/assignment counts" )
    {
        {  // Block, so we get dctor calls before function ends
            ...
            const NSArray<Counter> mc2(mc1);  // Copy ctor
    ...
    SUBCASE( "Move ctor: value-type ctor/dctor/assignment counts" )
    {
        {  // Block, so we get dctor calls before function ends
            ...
            const NSArray<Counter> mc2(move(mc1));  // Move ctor
