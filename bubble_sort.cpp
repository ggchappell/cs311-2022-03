...
// bubbleSort
// Sort a range using Bubble Sort.
...
void bubbleSort(RAIter first, RAIter last)
{
    ...
    {
        // Do one Bubble Sort pass on first[0 .. size-pass-1],
        ...
        for (size_t j = 0; j <= size-pass-2; ++j)
            // first[j], first[j+1] are current pair
        {
            // Compare & maybe swap current pair
            if (first[j+1] < first[j])  // Out of order?
            {
